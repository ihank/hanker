	...
	document.getElementById("content").addEventListener("click", function(e){
		...
		if(target.tagName == "INPUT"){
			...
			if(checked){
				...
				g_downResData[i] = [k, t, v, false,false]; //["script", "application/x-javascript","http://xxxx.js",null/blob, ispackaged];
            	...
            	chrome.tabs.sendMessage(tab.id, {msg: "getResource"},function(response) {
                	...
                	for(var key in data){
                		if(typeof key == "string"){
	        				...
	        				if(key == "img"){
	        					str += "<li><label><input type='checkbox' data-type='"+key+"' data-id='"+key+i+"' data-src='"+arr[i]+"'/><img src='"+arr[i]+"' /></label></li>";
	        				}else{
	        					str += "<li><label><input type='checkbox' data-type='"+key+"' data-id='"+key+i+"' data-src='"+arr[i]+"'/>" + arr[i] +"<a href='"+arr[i]+"' target='blank'></a></label></li>";
	        				}
